...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not a:
            return 1.0
        return min(1.0, max(0.0, len(a & c) / len(a)))
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        Returns:
            float in [0.0, 1.0]
        """
        a, q = _tokenize(answer), _tokenize(question)
        if not q:
            return 1.0
        return min(1.0, max(0.0, len(a & q) / len(q)))
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        Returns:
            float in [0.0, 1.0]
        """
        a, e = _tokenize(answer), _tokenize(expected)
        if not e:
            return 1.0
        return min(1.0, max(0.0, len(a & e) / len(e)))
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        if not contexts:
            return 0.0
        union = set().union(*[_tokenize(c) for c in contexts])
        return min(1.0, max(0.0, len(exp & union) / len(exp)))
    ...
    def evaluate_context_precision(
        ...
        for c in contexts:
            ...
            score = len(chunk_tokens & exp) / len(exp)
            relevance.append(score >= relevance_threshold)
        n_relevant = sum(relevance)
        if n_relevant == 0:
            return 0.0
        ap = sum(
            (sum(relevance[:k + 1]) / (k + 1)) * relevance[k]
    ...
    def run_full_eval(
        ...
        """
        faith = self.evaluate_faithfulness(answer, context or "")
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"Question: {question}\n\nAnswer: {answer}\n\nRubric:\n{rubric_text}\n\n"
            f"Score each criterion from 0.0 to 1.0 and return a JSON object "
            f"with criterion names as keys and float scores as values."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def avg_scores(batch_item: dict) -> float:
            vals = list(batch_item["scores"].values())
            return sum(vals) / len(vals) if vals else 0.0

        all_vals = [v for d in scores_batch for v in d["scores"].values()]
        overall_avg = sum(all_vals) / len(all_vals) if all_vals else 0.0

        if len(scores_batch) > 1:
            first_avg = avg_scores(scores_batch[0])
            rest_avg = sum(avg_scores(d) for d in scores_batch[1:]) / (len(scores_batch) - 1)
            positional = first_avg > rest_avg
        else:
            positional = False
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def avg(results_list, metric):
            vals = [getattr(r, metric) for r in results_list]
            return sum(vals) / len(vals) if vals else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_val = scores[lowest]
        tied = [k for k, v in scores.items() if v == min_val]
        if len(tied) > 1:
    ...
    def generate_improvement_suggestions(
        ...
        if "irrelevant" in categories:
            suggestions.append(
                "Improve prompt clarity to ensure answers address the question intent"
            )
        if "off_topic" in categories:
            suggestions.append(
                "Add intent detection to route off-topic questions to appropriate handlers"
            )
        # Always ensure at least 3 suggestions
        fallbacks = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement re-ranking to surface more relevant context chunks",
            "Add answer validation step to check factual consistency with retrieved context",
        ]
